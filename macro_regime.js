(()=>{
  ...
  function compute(macro,official,summary){
    ...
    const earningsScore=Math.round(cycle!=null?clamp(50+Number(cycle)*.5,0,100):50);
    let persistence='待確認';
  ...
  function render(r){const root=$('#macroRegimeRoot');if(!root)return;const d={growth:'GDP 成長結合 Elephant Cycle Score；高分代表景氣熱度明顯。',inflation:'月頻 Headline／核心 CPI、PPI 與動能；高分代表持續性通膨風險。',liquidity:'央行 M2 年增率；高分代表流動性仍偏寬鬆。',rates:'綜合通膨、成長與重貼現率的升息壓力。',earnings:'目前是 Cycle Score 盈利代理，不宣稱等同企業 EPS revision。'};root.innerHTML=`<div class="regime-hero"><div><span class="kicker">MACRO REGIME DETECTOR</span><h2>${esc(r.regime)}</h2><p>${esc(r.persistence)} · ${esc(r.sourceState)}</p></div><div class="hike-gauge"><span>升息壓力指標</span><b>${r.hike}</b><small>/100 · 非政策機率預測</small></div></div><div class="regime-grid">${card('Growth',r.scores.growth,d.growth)}${card('Inflation',r.scores.inflation,d.inflation)}${card('Liquidity',r.scores.liquidity,d.liquidity)}${card('Rates',r.scores.rates,d.rates)}${card('Earnings',r.scores.earnings,d.earnings)}</div><article class="panel regime-evidence"><div class="panel-head"><div><span class="kicker">EVIDENCE</span><h2>目前燈號依據</h2></div><span class="tag">deterministic / transparent</span></div><div class="table-wrap"><table><thead><tr><th>指標</th><th>最新值</th><th>解讀</th></tr></thead><tbody>${r.evidence.map(x=>`<tr><td>${esc(x[0])}</td><td>${esc(x[1])}</td><td>${esc(x[2])}</td></tr>`).join('')}</tbody></table></div></article><article class="notice"><strong>判讀邊界</strong><p>五燈是 regime detector，不是交易指令；升息壓力 0–100 也不是央行決策機率。缺資料時 fail closed 顯示未接入，不以估值補洞。Earnings 目前明確標示為 Cycle Score proxy，待後續接真正 earnings revision 後再升級。</p></article>`}
  async function init(){const root=$('#macroRegimeRoot');if(!root)return;try{const [macro,official,summary]=await Promise.all([loadJson('data/macro.json'),loadJson('data/regime_official.json').catch(()=>({series:{}})),loadJson('data/summary.json').catch(()=>null)]);render(compute(macro,official,summary))}catch(e){root.innerHTML=`<article class="notice"><strong>Macro Regime 載入失敗</strong><p>${esc(e.message)}</p></article>`}}
  init();
})();
