...
opp=capital_allocation.opportunity(alpha,inputs,portfolio);r=capital_allocation.lifecycle(alpha,opp,policy,portfolio,1.0);assert r[0]['portfolio_action']=='RESEARCH'
a2=copy.deepcopy(alpha);a2['stocks'][0]['action']='BUY CANDIDATE';r=capital_allocation.lifecycle(a2,opp,policy,portfolio,1.0);assert r[0]['portfolio_action']=='BUY_REVIEW'
d=capital_allocation.distribution(a2['stocks'][0],30);assert d['status']=='COMPLETE' and d['probability_beating_hurdle_pct']==75.0
q=capital_allocation.research_queue({'meta':{'status':'COMPLETE'},'deep_research_queue':[{'ticker':'2408','name':'南亞科','industry':'24'}]},{'stocks':[]});assert q['items'][0]['promotion_authority']=='NONE'
