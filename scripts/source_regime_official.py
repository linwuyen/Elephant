...
CBC_RATE_PAGES = [
    'https://www.cbc.gov.tw/tw/lp-640-1-1-60.html',
]
...
def _resolve_resource(meta_url: str, dataset_id: str) -> str:
    body = decode_text(request_bytes(meta_url)[0])
    obj = json.loads(body)
    urls = []
    ...
    def walk(x):
        if isinstance(x, dict):
            ...
            if '.xml' in x.lower() or 'download' in x.lower():
...
def _yoy_from_index(data):
    ...
    for p, value in data:
        y, m = map(int, p.split('-'))
        prev = vals.get(f'{y - 1:04d}-{m:02d}')
        if prev not in (None, 0):
            out.append([p, (float(value) / float(prev) - 1.0) * 100.0])
    return out
...
def _select_dgbas_yoy(body: bytes, item_re: str):
    ...
    level = []
    for r in matched:
        p = _month(r.get('TIME_PERIOD'))
        v = num(r.get('Item_VALUE'))
...
def parse_core_cpi(body: bytes):
    ...
    return {
        'dgbas.cpi.core_yoy': {
            'name': '核心消費者物價指數年增率', 'unit': 'percent', 'frequency': 'monthly',
            'dataset_id': 'data.gov.tw/8237', 'data': data,
        }
    }
...
def parse_ppi(body: bytes):
    ...
    return {
        'dgbas.ppi.yoy': {
            'name': '生產者物價指數年增率', 'unit': 'percent', 'frequency': 'monthly',
            'dataset_id': 'data.gov.tw/148439', 'data': data,
        }
    }


def parse_cbc_m2(text: str):
    pairs = re.findall(r'(20\d{2})[./-](\d{2})\s*(?:</[^>]+>\s*)*\|?\s*([+-]?\d+(?:\.\d+)?)', text)
    if not pairs:
        plain = re.sub(r'<[^>]+>', ' ', text)
    ...
    return {
        'cbc.m2.yoy': {
            'name': '貨幣總計數 M2 年增率', 'unit': 'percent', 'frequency': 'monthly',
            'dataset_id': 'cbc.lp-643', 'data': data,
        }
    }
...
def parse_cbc_rates(text: str):
    ...
    for y, m, d, rate in pairs:
        data.append([f'{int(y):04d}-{int(m):02d}', float(rate)])
    data = _dedup(data)
    if not data:
        raise ValueError('CBC discount-rate history empty')
    return {
        'cbc.discount_rate': {
            'name': '中央銀行重貼現率', 'unit': 'percent', 'frequency': 'event/month',
            'dataset_id': 'cbc.lp-640', 'data': data,
        }
    }
...
def update(offline=None):
    ...
    out = {
        'generated_from': 'first_party_official_sources',
        'contract': 'macro-regime-official-v1',
        'series': series,
        'latest_period': latest,
    }
    save_json('regime_official.json', out)
    return {'latest_period': latest, 'rows': sum(len(s['data']) for s in series.values()), 'message': 'DGBAS core CPI/PPI + CBC M2/discount rate refreshed'}
