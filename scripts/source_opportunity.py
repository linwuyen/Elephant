...
def parse_0050(text):
 # Yuanta is server rendered; parse the visible page, but keep raw HTML/JSON as a fallback.
 candidates=(plain(text),raw_normalized(text))
 pe=asof=None
 for t in candidates:
...
def parse_vt(text):
 # Vanguard may hydrate characteristics inside script/JSON rather than visible markup.
 # Search both rendered text and raw serialized payload; require the P/E label so a price
 # or another ratio cannot be mistaken for valuation.
 candidates=(plain(text),raw_normalized(text))
 pe=asof=None
 for t in candidates:
  ...
  pats=(
   r'P\s*/\s*E\s+ratio\D{0,160}?([0-9]+(?:\.[0-9]+)?)\s*x?',
   r'P/E\s*ratio.{0,400}?(?:value|displayValue|fundValue)["\s:=\\]+([0-9]+(?:\.[0-9]+)?)',
   r'(?:label|name)["\s:=\\]+P/E\s*ratio.{0,500}?([0-9]+(?:\.[0-9]+)?)\s*x?',
  )
  for p in pats:
   mp=re.search(p,t,re.I|re.S)
   if mp:
    pe=float(mp.group(1));break
