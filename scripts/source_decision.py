...
NCCC_CARD_URL = 'https://www.nccc.com.tw/dataDownload/NCCC%20Open%20Data/Location/NCCC_TWN.CSV'
NCCC_CARD_CATALOG = 'https://data.gov.tw/dataset/25364'
...
def parse_long_moea(body, url, kind):
    ...
    for r in rows:
        ...
        if kind == 'orders':
            ...
            label = ' / '.join(
                x for x in [
                    str(pick(r, '貨品別') or '').strip(),
                    str(pick(r, '地區別') or '').strip(),
                    str(pick(r, '統計項目', '項目') or '').strip(),
                ] if x
            )
        else:
    ...
    return {
        k: {'name': k, 'unit': unit, 'data': dedup(v)}
...
def parse_customs(body):
    rows = decode_resource(body, CUSTOMS_URL)
    data = []
    for r in rows:
        y = num(pick(r, '年度', '年'))
        m = num(pick(r, '月份', '月'))
        ...
        if yy < 1911:
            yy += 1911
        if 1 <= mm <= 12:
            data.append([f'{yy:04d}-{mm:02d}', v])
    data = dedup(data)
    if len(data) < 24:
        raise ValueError('customs exports parse returned too few rows')
    return {'customs.exports_total': {'name': '海關出口總值', 'unit': 'thousand_ntd', 'data': data}}


def parse_credit_card(body):
    text = decode_text(body).lstrip('\ufeff')
    data = []
    for row in csv.DictReader(text.splitlines()):
        area = str(row.get('地區') or '').strip()
        if area not in ('臺灣', '台灣'):
            continue
        rawp = str(row.get('年月') or '').strip()
        m = re.fullmatch(r'(\d{4})(\d{2})', rawp)
        if not m:
            continue
        year, month = int(m.group(1)), int(m.group(2))
        if not 1 <= month <= 12:
            continue
        amount_key = next((k for k in row if '處理金額' in str(k)), None)
        amount = num(row.get(amount_key)) if amount_key else None
        if amount is not None:
            data.append([f'{year:04d}-{month:02d}', amount])
    data = dedup(data)
    if len(data) < 24:
        raise ValueError('NCCC card spending parse returned too few rows')
    return {
        'nccc.card_spending': {
            'name': '聯卡中心處理信用卡消費金額',
            'unit': 'ntd',
            'data': data,
            'coverage_note': '聯合信用卡處理中心處理之信用卡消費交易，作為信用卡消費 proxy，非所有支付行為。',
        }
    }


def parse_labor(body):
    rows = decode_resource(body, LABOR_URL)
    specs = {
    ...
    names = {
        ...
        'avg_monthly_salary': '工業及服務業平均月薪資',
        'manufacturing_monthly_salary': '製造業平均月薪資',
        'avg_monthly_hours': '工業及服務業平均月工時',
        'cpi_yoy': 'CPI 年增率',
    }
    units = {
        'unemployment_rate': 'percent',
        'avg_monthly_salary': 'ntd',
        'manufacturing_monthly_salary': 'ntd',
...
def parse_month_any(raw):
    ...
    if m and int(m.group(1)) < 1911 and 1 <= int(m.group(2)) <= 12:
...
def parse_dgbas_long(body, name):
    ...
    for r in xml_leaf_rows(body):
        ...
        if not pkey or not vkey:
            continue
        p = parse_month_any(r.get(pkey))
        v = num(r.get(vkey))
        ...
        label = ' / '.join(
            str(v0)
            for k, v0 in r.items()
            if k not in (pkey, vkey) and ('category' in k.lower() or '項目' in k or '行業' in k)
        )
        groups.setdefault(label or 'total', []).append([p, v])
    if not groups:
        raise ValueError('DGBAS long XML parse empty')

    def rank(item):
        label, data = item
        x = label.lower()
        score = len(data)
        if '工業及服務業' in label or 'industry and services' in x:
            score += 10000
        if '總計' in label or 'total' in x:
            score += 3000
        if '製造業' in label:
            score -= 1000
        return score

    label, data = max(groups.items(), key=rank)
    data = dedup(data)
    if len(data) < 24:
        raise ValueError('DGBAS wage history too short')
...
def parse_dgbas_employment(body):
    ...
    for r in xml_leaf_rows(body):
        ...
        vkey = next(
            (
                k for k in r
                if ('工業及服務業' in k or 'industry_and_services' in k.lower())
                and ('人數' in k or 'person' in k.lower())
            ),
            None,
        )
        if not vkey:
    ...
    data = dedup(data)
    if len(data) < 24:
        raise ValueError('DGBAS employment parse too short')
    return {'dgbas.employment_total': {'name': '工業及服務業受僱員工人數', 'unit': 'persons', 'data': data}}


def merge_recent(base, recent):
    d = dict((base or {}).get('data', []))
    d.update(dict((recent or {}).get('data', [])))
    out = dict(base or recent or {})
    out['data'] = dedup(d.items())
    return out
    ...
    for row in csv.reader(decode_text(body).splitlines()):
        ...
        for key, idx in columns.items():
            if idx < len(row):
                v = num(row[idx])
                if v is not None:
                    out[key].append([period, v])
    return {k: dedup(v) for k, v in out.items() if len(dedup(v)) >= 12}


def parse_cbc_financial(bodies):
    raw = {
        **cbc_month_rows(bodies['money'], {'m1b_yoy': 16, 'm2_yoy': 20}),
        **cbc_month_rows(bodies['credit'], {'credit_yoy': 20}),
        **cbc_month_rows(bodies['markets'], {'interbank_rate': 7, 'exchange_rate': 11}),
    }
    names = {
        'm1b_yoy': 'M1B 年增率',
        'm2_yoy': 'M2 年增率',
        'credit_yoy': '金融機構放款與投資年增率',
        'interbank_rate': '金融業隔夜拆款加權平均利率',
        'exchange_rate': '銀行間美元收盤匯率',
    }
    units = {
        'm1b_yoy': 'percent',
        'm2_yoy': 'percent',
        'credit_yoy': 'percent',
        'interbank_rate': 'percent',
        'exchange_rate': 'ntd_per_usd',
    }
    out = {k: {'name': names[k], 'unit': units[k], 'data': v} for k, v in raw.items()}
    for required in ('m1b_yoy', 'm2_yoy', 'credit_yoy', 'interbank_rate', 'exchange_rate'):
        if required not in out:
            raise ValueError('CBC monthly table missing ' + required)
    for key in ('m1b_yoy', 'm2_yoy', 'credit_yoy'):
        if not all(-30 < float(v) < 50 for _, v in out[key]['data'][-24:]):
            raise ValueError('CBC implausible ' + key)
    if not all(0 < float(v) < 20 for _, v in out['interbank_rate']['data'][-24:]):
        raise ValueError('CBC implausible interbank_rate')
    if not all(20 < float(v) < 50 for _, v in out['exchange_rate']['data'][-24:]):
        raise ValueError('CBC implausible exchange_rate')
    return out


def update(_offline=None):
    old = load_json('decision_inputs.json', {'series': {}})
    series = dict(old.get('series', {}))
    warnings = []
    rows = 0

    for name, url, kind, prefix in [
        ('orders', ORDERS_URL, 'orders', 'orders.'),
        ('domestic', DOMESTIC_URL, 'domestic', 'domestic.'),
        ('inventory', INVENTORY_URL, 'inventory', 'inventory.'),
    ]:
        try:
            body, _ = request_bytes(url, 75, 3)
            parsed = parse_long_moea(body, url, kind)
            rows += sum(len(x['data']) for x in parsed.values())
            if not parsed:
                raise ValueError(f'{name} parse empty')
    ...
    try:
        body, _ = request_bytes(CUSTOMS_URL, 60, 3)
        parsed = parse_customs(body)
        rows += sum(len(x['data']) for x in parsed.values())
        series.update(parsed)
    except Exception as e:
        warnings.append(f'customs: {type(e).__name__}: {e}')

    try:
        body, _ = request_bytes(NCCC_CARD_URL, 60, 3)
        parsed = parse_credit_card(body)
        rows += sum(len(x['data']) for x in parsed.values())
        series.update(parsed)
    except Exception as e:
        warnings.append(f'nccc_credit_card: {type(e).__name__}: {e}')

    labor = {}
    try:
        body, _ = request_bytes(LABOR_URL, 60, 3)
        labor = parse_labor(body)
        rows += sum(len(x['data']) for x in labor.values())
        if not labor:
            raise ValueError('labor parse empty')
        for k, v in labor.items():
            series['labor.' + k] = v
    except Exception as e:
        warnings.append(f'labor: {type(e).__name__}: {e}')

    try:
        body, _ = request_bytes(DGBAS_WAGE_URL, 75, 2)
        parsed = parse_dgbas_long(body, '工業及服務業平均每月總薪資')
        rows += sum(len(x['data']) for x in parsed.values())
        series.update(parsed)
        if labor.get('avg_monthly_salary'):
            series['dgbas.total_monthly_salary'] = merge_recent(
                series['dgbas.total_monthly_salary'], labor['avg_monthly_salary']
            )
    except Exception as e:
        warnings.append(f'dgbas_wage: {type(e).__name__}: {e}')

    try:
        body, _ = request_bytes(DGBAS_EMP_URL, 75, 2)
        parsed = parse_dgbas_employment(body)
        rows += sum(len(x['data']) for x in parsed.values())
        series.update(parsed)
    except Exception as e:
        warnings.append(f'dgbas_employment: {type(e).__name__}: {e}')

    try:
        bodies = {key: request_bytes(url, 60, 3)[0] for key, url in CBC_TABLES.items()}
        parsed = parse_cbc_financial(bodies)
        rows += sum(len(x['data']) for x in parsed.values())
        for k, v in parsed.items():
            series['cbc.' + k] = v
    except Exception as e:
        warnings.append(f'cbc_financial: {type(e).__name__}: {e}')
    ...
    catalogs = {
        ...
        'labor': LABOR_CATALOG,
        'dgbas_wage': DGBAS_WAGE_CATALOG,
        'dgbas_employment': DGBAS_EMP_CATALOG,
        'nccc_credit_card': NCCC_CARD_CATALOG,
        'cbc_financial': CBC_FIN_CATALOG,
    }
    obj = {
        'source': 'Taiwan official decision inputs',
    ...
    if not series:
        raise ValueError('no decision input series available')
    return {
        'latest_period': latest,
        'rows': rows,
        'message': 'official decision inputs refreshed' + (('; warnings: ' + '; '.join(warnings)) if warnings else ''),
        'warnings': warnings,
    }
