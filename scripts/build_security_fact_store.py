...
def completeness(official,evidence):
 req={'reference_price','earnings_basis','revenue_trend','balance_sheet_cash_flow','material_events','valuation_basis'};have=set(evidence.get('metrics') or [])
 if official.get('income_statement'):have.add('earnings_basis')
 if official.get('monthly_revenue'):have.add('revenue_trend')
 if official.get('balance_sheet'):have.add('balance_sheet_cash_flow')
 return {'required':sorted(req),'available':sorted(req&have),'missing':sorted(req-have),'coverage_pct':round(len(req&have)/len(req)*100,1)}
...
def generate():
 ...
 out={'version':2,'source':'Alpha upstream + deterministic TWSE/MOPS OpenAPI normalized fact store','as_of':alpha.get('meta',{}).get('as_of'),'securities':rows,'official_source_health':load_json('security_official_facts.json',{}).get('source_health',{}),'guardrail':'Fact store normalizes evidence and official numeric facts; missing facts remain explicit and it has no BUY authority.'};out['fingerprint']=digest(out['securities']);save_json('security_fact_store.json',out);return out
