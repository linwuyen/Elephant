...
def sales_index():
 ...
 rows,url=dataset_rows(SALES_DATASET);series=parse(rows,('統計值(指數)','統計值'),('銷售量指數','銷售指數'))
 if not series:raise ValueError('data.gov MOEA sales index parse empty')
 total=series.get('C') or next((x for x in series.values() if str(x.get('name','')).replace(' ','')=='製造業'),None)
 if not total or len(total.get('data',[]))<5:raise ValueError('data.gov MOEA sales manufacturing-total series missing/short')
 if not all(0<float(v)<1000 for _,v in total['data'][-24:]):raise ValueError('data.gov MOEA sales values outside plausible range')
 return {'indicator_id':'moea.manufacturing.sales_index_current','name':'製造業銷售指數（政府資料開放平台官方資源）','unit':infer_unit(rows,'index_2021_100'),'series':series,'source_url':url,'catalog':CATALOGS[SALES_DATASET],'transport':'data.gov metadata → MOEA CSV/ZIP'}
...
if __name__=='__main__':
 s=sales_index();print('sales',len((s['series'].get('C') or {}).get('data',[])),s['source_url']);print('inventory',update_inventory())
