...
ndc_latest = status.get('sources', {}).get('ndc', {}).get('latest_period')
ndc_age = month_age(ndc_latest)
if ndc_age is None or ndc_age > 4:
    errors.append(f'ndc: stale business-cycle data, latest={ndc_latest}, age_months={ndc_age}')
lead = ndc.get('series', {}).get('leading_no_trend', {}).get('data', [])
coinc = ndc.get('series', {}).get('coincident_no_trend', {}).get('data', [])
if not lead or not coinc:
    errors.append('ndc: leading/coincident series missing')
