...
MODERN_SALES_VOLUME_ZIP='https://dmz9.moea.gov.tw/gmweb/opendata/%E7%B6%93%E6%BF%9F%E9%83%A8%E7%B5%B1%E8%A8%88%E8%99%95_%E8%A3%BD%E9%80%A0%E6%A5%AD%E9%8A%B7%E5%94%AE%E9%87%8F%E6%8C%87%E6%95%B8.zip'
...
def month_period(value):
    ...
    if len(digits)==6 and int(digits[:4])>=1911:
        ...
        year=int(digits[:-2]); month=int(digits[-2:])
        if year<1911: year+=1911
    elif len(digits)==4:
        # Current open-data feeds may encode ROC year/month as 1156.
        year=int(digits[:3])+1911; month=int(digits[3:])
    else:
        raise ValueError(f'unsupported month period: {value!r}')
...
def parse(rows,value_cols,aliases=()):
    ...
    grouped={}
    for r in source:
        raw_period=pick(r,'資料期(民國年)','資料期','年月','年月份')
        v=value_of(r)
...
def update(offline=None):
    ...
    specs=[
      ...
      ('moea.manufacturing.sales_volume','moea_manufacturing_sales_volume_index.csv',MODERN_SALES_VOLUME_ZIP,'moea.manufacturing.sales_volume_index','製造業銷售指數','index_2021_100',('銷售量指數','銷售指數'),('統計值(指數)','統計值')),
    ...
    for ds,fn,url,iid,name,fallback,aliases,vcols in specs:
        rows=resource_rows(offline,fn,url); total+=len(rows)
        series=parse(rows,vcols,aliases)
        if not series:
    ...
    return {'latest_period':max_period(out),'rows':total,'message':'MOEA official d/f/ec CSV + current sales-index ZIP refreshed'}
