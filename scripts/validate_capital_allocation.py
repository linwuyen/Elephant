...
policy=load('portfolio_policy.json');state=load('portfolio_state.json');inputs=load('opportunity_inputs.json');capital=load('capital_allocation.json');rq=load('deep_research_queue.json');opp=load('opportunity_set.json');bundle=load('alpha_engine.json');alpha=bundle.get('alpha',{});screen=bundle.get('screen',{})
if capital.get('version')!=1 or not capital.get('fingerprint'):fail('capital version/fingerprint')
...
up={str(x.get('ticker')):x.get('action') for x in alpha.get('stocks',[])}
for x in capital.get('lifecycle',[]):
 t=str(x.get('ticker'));pa=x.get('portfolio_action');ua=x.get('upstream_action')
 if up.get(t)!=ua:fail('upstream action mismatch '+t)
 if ua!='BUY CANDIDATE' and pa in ('BUY_REVIEW','ADD_REVIEW'):fail('portfolio upgraded '+t)
