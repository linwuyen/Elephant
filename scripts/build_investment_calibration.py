...
def generate():
 ...
 final={'version':2,'updated_at':now.isoformat(),'decisions':records,'resolved_horizon_count':sum(len(x.get('outcomes') or {}) for x in records),'primary_kpi':'realized_opportunity_cost_alpha','attribution_note':'EPS/multiple/timing decomposition remains BLOCKED until point-in-time realized EPS and multiple facts exist; no synthetic attribution is fabricated.','guardrail':'Prospective decision inputs, including Constitution status/gates, are immutable; only future outcome fields may be appended.'};save_json('investment_calibration.json',final);return final
