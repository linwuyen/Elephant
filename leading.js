(()=>{
  ...
  function download(){
    ...
    const csv='\ufeff'+[['period','value'],...s.data].map(r=>r.map(q).join(',')).join('\n');
    const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`ndc-${sel.value}.csv`;a.click();URL.revokeObjectURL(a.href);
  }
  ...
  async function loadNDC(){
    try{
      ...
      sel.onchange=draw; $('#leadingCsv')?.addEventListener('click',download);
