(()=>{
  ...
  function card(key,x,idx,hasHistory){
    ...
    return `<article class="decision-card ${cls}"><div class="top"><div><div class="meta">${esc(cfg[key].name)}</div><div class="label">${esc(cfg[key].question)}</div></div><div><div class="score">${score}</div><div class="meta">Confidence ${confidence}</div></div></div><div class="bar ${isConcentration?'concentration-bar':'direction-bar'}"><div class="fill" style="width:${width}%"></div>${thresholdTicks}</div>${scoreAxis(isConcentration)}<div class="meta">${esc(x.period||'—')} · ${esc(x.label||'')}</div>${meaning}<details class="decision-components"><summary>查看 Score 元件</summary>${(x.components||[]).map(c=>component(c,key)).join('')}</details>${history}</article>`;
  }
